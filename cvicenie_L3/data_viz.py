...
def aggregate_data(data):
    ...
    for day in data["Day"].unique():
        ...
        for hour in data["Sample"].unique():
            agg_data.loc[agg_data["Hour"] == hour, "defects_day_" + str(day)] = data[
                (data["Day"] == day) & (data["Sample"] == hour)
            ]["Defects"].values
    return agg_data
...
st.write(
    "Data Source: [Kaggle Manufacturing Defects Dataset](https://www.kaggle.com/datasets/gabrielsantello/manufacturing-defects-industry-dataset)"
)
...
if show_seaborn:
    ...
    sns.heatmap(
        agg_data.iloc[:, 1:].astype(float).T,
        cmap=color_scheme,
        annot=True,
        fmt=".1f",
...
if show_timeline:
    ...
    timeline_data["DateTime"] = pd.to_datetime(
        "2025-09-"
        + timeline_data["Day"].astype(str).str.zfill(2)
        + " "
        + timeline_data["Sample"]
    )
    timeline_data["DayLabel"] = "Day " + timeline_data["Day"].astype(str)
    timeline_data["HourMinute"] = (
        pd.to_datetime(timeline_data["Sample"], format="%H:%M").dt.hour
        + pd.to_datetime(timeline_data["Sample"], format="%H:%M").dt.minute / 60.0
    )
    ...
    fig_timeline.layout.updatemenus[0].buttons[0].args[1]["frame"][
        "duration"
    ] = 1000  # 1 second per frame
    fig_timeline.layout.updatemenus[0].buttons[0].args[1]["transition"][
        "duration"
    ...
    daily_stats = (
        timeline_data.groupby("Day")["Defects"]
    ...
    fig_trend = px.line(
        ...
        labels={"value": "Number of Defects", "variable": "Statistic", "Day": "Day"},
        color_discrete_map={"mean": "blue", "max": "red", "min": "green"},
    )
