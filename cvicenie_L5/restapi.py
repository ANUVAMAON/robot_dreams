...
def fetch_weather_data(latitude, longitude, days):
    ...
    try:
        print(f"Fetching historical weather data for coordinates: {latitude}, {longitude}")
...
def create_plotly_visualizations(df):
    ...
    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=('Temperature Over Time', 'Temperature vs Humidity Scatter', 
                       'Daily Temperature Range', 'Weather Parameters Comparison'),
    ...
    fig.add_trace(
        go.Scatter(x=df['datetime'], y=df['temperature_celsius'], 
    ...
    fig.add_trace(
        go.Scatter(x=df['temperature_celsius'], y=df['humidity_percent'],
                  mode='markers', name='Temp vs Humidity',
                  marker=dict(color=df['wind_speed_kmh'], colorscale='Viridis',
    ...
    daily_temps = df.groupby('date')['temperature_celsius'].agg(['min', 'max', 'mean']).reset_index()
    
    fig.add_trace(
        go.Scatter(x=daily_temps['date'], y=daily_temps['max'], 
                  name='Max Temp', line=dict(color='red')),
        row=2, col=1
    )
    fig.add_trace(
        go.Scatter(x=daily_temps['date'], y=daily_temps['min'], 
                  name='Min Temp', fill='tonexty', line=dict(color='blue')),
        row=2, col=1
    )
    fig.add_trace(
        go.Scatter(x=daily_temps['date'], y=daily_temps['mean'], 
    ...
    fig.add_trace(
        go.Scatter(x=df['datetime'], y=df['humidity_percent'], 
                  name='Humidity (%)', line=dict(color='blue')),
        row=2, col=2, secondary_y=True
    )
    fig.add_trace(
        go.Scatter(x=df['datetime'], y=df['wind_speed_kmh'], 
    ...
    fig.update_yaxes(title_text="Humidity (%) / Wind Speed (km/h)", row=2, col=2, secondary_y=True)
...
def main():
    ...
    raw_data = fetch_weather_data(latitude=48.7205, longitude=21.2578, days=10) #Košice
